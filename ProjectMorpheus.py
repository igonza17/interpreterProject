#Fully Functioning program DO NOT EDIT
import string
#Error Handling--------------------------------------------------------------------------------------------
class Errors:
...
class runtimeErr(Errors):
    ...
    def generateTraceback(self):
        ...
        while context:
            result = f'line {str(position.line + 1)},in {context.displayName}\n' + result
...
#Tokens----------------------------------------------------------------------------------------------------
INT, FLOAT, PLUS, MINUS, MUL, DIV, LPAREN, RPAREN, EOF, IDENTIFIER, KEYWORD, EQUALS = 'INT', 'FLOAT', 'PLUS', 'MINUS', 'MUL', 'DIV', 'LPAREN', 'RPAREN', 'EOF', 'IDENTIFIER', 'KEYWORD', 'EQUALS'
# EQUAL2, NOTEQUAL, LESSTHAN, GREATERTHAN, LESSTHANEQUAL, GREATERTHANEQUAL = 'EQUAL2','NOTEQUAL','LESSTHAN','GREATERTHAN','LESSTHANEQUAL','GREATERTHANEQUAL'
EE = 'EE'
NE = 'NE'
LT = 'LT'
GT = 'GT'
...
class lexer:
    ...
    def makeTokens(self):
        ...
        while self.currentToken != None:
            if self .currentToken in ' \t':
                ...
                self.advance()
                return [],IllegalChar(posStart, self.pos, "'" + char + "'")
    ...
    def makeNotEqual(self):
        ...
        self.advance()
        return None, expectedChar(posStart,self.pos, "'=' goes after '!' ")
...
class unaryOperationNode:
    def __init__(self,opToken,node):
        ...
        self.posEnd = self.opToken.posEnd
...
class parser:
    ...
    def parse(self):
        ...
        if not result.error and self.currentToken.type != EOF:
            return result.failure(IllegalSyntax(self.currentToken.posStart,self.currentToken.posEnd,"Missing or Expected +,-,*,/,==,!=,<,>,<=,>=,and,or operators "))
        return result
    ...
    def ifExpr(self):
        ...
        if not self.currentToken.matches(KEYWORD,'if'):
            return res.failure(IllegalSyntax(self.currentToken.posStart,self.currentToken.posEnd,"Missing or Expected 'if' "))
        ...
        if not self.currentToken.matches(KEYWORD,'then'):
            return res.failure(IllegalSyntax(self.currentToken.posStart, self.currentToken.posEnd,"Missing or Expected 'then' "))
        res.resgisterAdvance()
        self.advance()
    ...
    def atom(self):
        ...
        if tok.type in (INT,FLOAT):
            ...
            if self.currentToken.type == RPAREN:
                ...
                return res.failure(IllegalSyntax(self.currentToken.posStart,self.currentToken.posEnd,"Missing or Expected ')' "))
        elif tok.matches(KEYWORD,'if'):
            ifExpr = res.register(self.ifExpr())
            if res.error:return res
            return res.success(ifExpr)
        return res.failure(IllegalSyntax(tok.posStart,tok.posEnd,"Missing or Expected identifier, int, float, +, -, or '(' "))
    ...
    def comparisonExpr(self):
        ...
        if res.error:
            return res.failure(IllegalSyntax(self.currentToken.posStart,self.currentToken.posEnd,"Missing or Expected identifier, int, float,identifier, +, -, or '(' , 'not'" ))
    ...
    def expr(self):
        ...
        if self.currentToken.matches(KEYWORD,'var'):
            ...
            if self.currentToken.type != IDENTIFIER:
                return res.failure(IllegalSyntax(self.currentToken.posStart,self.currentToken.posEnd,"Missing or Expected Identifier"))
        ...
        if res.error:
            return res.failure(IllegalSyntax(self.currentToken.posStart,self.currentToken.posEnd,"Missing or Expected 'var','int','float','identifier','+','-' ,'(' "))
        return res.success(node)
...
class Number:
    ...
    def division(self,other):
        if isinstance(other,Number):
            if other.value == 0:
                return None, runtimeErr(other.posStart,other.posEnd,'Division by zero',self.context)
            return Number(self.value / other.value).setContext(self.context), None
...
class eval:
    ...
    def visit_varAccessNode(self,node,context):
        ...
        if not value:
            return res.failure(runtimeErr(node.posStart, node.posEnd,f"'{varName}' is not defined",context))
    ...
    def visit_unaryOperationNode(self,node,context):
        ...
        elif node.opToken.matches(KEYWORD,'not'):
